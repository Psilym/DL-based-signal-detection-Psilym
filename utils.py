...
def get_pf(x_val,y_val,val_SNRs,model,epoch,pf_min,pf_max):
    ...
    if (pf>pf_min) and (pf<pf_max):
        print("Pf meet the threshold, training stopped")
        model.stop_training=True    

def myperformance_evaluation(x_test_snr,y_test_snr,model,select_snr):
    '''
        Evaluate final model's performance
    '''
    y_test_hat_snr = model.predict(x_test_snr,verbose=0)

    cm = confusion_matrix(np.argmax(y_test_snr, 1), np.argmax(y_test_hat_snr,1))
    # Pd_norm = Pd.astype('float') / Pd.sum(axis=1)[:, np.newaxis]
    # Pd_norm = np.nan_to_num(Pd_norm)
    tn = cm[0][0]
    fn = cm[1][0]
    fp = cm[0][1]
    tp = cm[1][1]
    sum_num = tn + fn + tp + fp
    accuracy = (tn + tp)/sum_num
    Pf = fp/(tn+fp)
    Pd = tp/(fn+tp)
    # Pf_final = Pd_norm[1][0]
    # my evaluation
    print("SNR=",select_snr,",Pd:",Pd,",Pf:",Pf,",Accuaracy:",accuracy)
...
def performance_evaluation(save_path,x_test,y_test,test_SNRs,model):
    ...
    img_plt,pf=getConfusionMatrixPlot(np.argmax(y_test,1),np.argmax(y_test_hat,1))
    plt.show()
    pd_list=[]
    snrs = np.arange(-50,10,2)
    ...
    for snr in snrs:
        ...
        if test_x_i.shape[0] == 0:
            pd_list.append(0.123)
            print("SNR=",snr,",Pd:",0.123)
            continue
        else:
...
def radioml_IQ_data_snr(filename, select_snr):
    '''
    ...
    lbl = np.array(lbl)
    index = np.where(lbl == 'BPSK')[0]
    X = X[index]
    lbl = lbl[index]
    index = np.where(lbl == f'{select_snr}')[0]
    X = X[index]
    lbl = lbl[index]
    ...
    #     real = X[i,0,:]
    #     imag = X[i,1,:]
    #     x_complex = real + 1j*imag
    #     energy = np.sum(np.abs(x_complex) ** 2)
    #     x = x_complex / (energy ** 0.5)
    #     real = np.real(x)
    #     imag = np.imag(x)
    #     X_my.append([real, imag])
    # X_my = np.array(X_my)
    # X = X_my
    ...
    shuffle_idx2 = np.random.choice(range(0, total_num), size=total_num, replace=False)
    ...
    x_train_snr = dataset[0:train_num]
    y_train_snr = labelset[0:train_num]
    train_SNRs_snr = SNR[0:train_num]
    x_val_snr = dataset[train_num:train_num + val_num]
    y_val_snr = labelset[train_num:train_num + val_num]
    val_SNRs_snr = SNR[train_num:train_num + val_num]
    x_test_snr = dataset[train_num + val_num:]
    y_test_snr = labelset[train_num + val_num:]
    test_SNRs_snr = SNR[train_num + val_num:]

    # x_train_snr = x_train[np.where(train_SNRs==select_snr)]
    # y_train_snr = y_train[np.where(train_SNRs==select_snr)]
    # x_val_snr = x_val[np.where(val_SNRs==select_snr)]
    # y_val_snr = y_val[np.where(val_SNRs==select_snr)]
    # x_test_snr = x_test[np.where(test_SNRs==select_snr)]
    # y_test_snr = y_test[np.where(test_SNRs==select_snr)]

    print("Training data:", x_train_snr.shape)
    print("Training labels:", y_train_snr.shape)
    print("Validation data:", x_val_snr.shape)
    print("Validation labels:", y_val_snr.shape)
    print("Testing data", x_test_snr.shape)
    print("Testing labels", y_test_snr.shape)

    return x_train_snr, y_train_snr, x_val_snr, y_val_snr, x_test_snr, y_test_snr
